...
@router.post("/create")
def branch(
    namespace: str = Query(..., description="Namespace (e.g. 'branch')"),
    table_name: str = Query(..., description="Table (e.g. 'branch-data')"),
    ...
    rows = mssql_branch((start_fmt), (end_fmt))
...
def get_data(
    namespace: str = Query(..., description="Namespace (e.g. 'invoice')"),
    table_name: str = Query(..., description="Table name (e.g. 'invoice-branch')"),
    credentials: HTTPAuthorizationCredentials = Depends(security)
):
    verify_jwt(credentials.credentials)
    try:
